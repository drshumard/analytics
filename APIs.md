...
`increment` body: `{ "field": "registrations", "count": 1, "email": "...", "name": "...", "phone": "..." }`. Valid `field`: `fb_spend`, `fb_link_clicks`, `registrations`, `replays`, `viewedcta`, `clickedcta`, `purchases`, `attended`, `stayeduntil`. Purchases route by `metadata.source` (`Paid Ads`/`Native`/`Youtube`/`AI Bot`/`AI Bot B`/`CPA Traffic`/`Sales A`/`Sales B`/`Retargeting`/`Promo`/`Post Webinar`; an explicit `Post Webinar` is trusted verbatim, otherwise Paid Ads / Sales A / Sales B sales auto‑detect Post‑Webinar (12h+ after last webinar engagement)). Registrations dedupe by email + webinar day; other events dedupe within 5 minutes. Each event is also written to the `events` table.
...
| `GET` | `/api/ai/tools/<email>` | Shortcut: a contact's journey, **condensed for bots** — identity, attribution, stage, funnel events and linked identities in full; pageviews rolled up per page (`visits` count + first/last seen); timeline **newest-first** so the freshest signal survives client-side truncation. The `@` marks it as an email, no body needed. Requires `get_contact_journey` in scope; also works as `POST`. For the full uncondensed journey, `POST /api/ai/tools/get_contact_journey` with `{"identifier":"<email>"}`. |
